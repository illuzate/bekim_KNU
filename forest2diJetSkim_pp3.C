...
//  TTree *trackTree = (TTree*)f1->Get("anaTrack/trackTree");
  ...
  TFile* newfile = new TFile(Form("skimmedFiles/TrkCutsppreco_%s_trig%s_jetCollection%s_minJetPt%d_%s.root",outputFname.Data(), trig.Data(), jetCollection.Data(), (int)minjPt, dayTime.Data()  ),"recreate");
...
*/
     ...
     if(numjt > 1) {     // if numjt > 1 
       sort(orderpt.begin(), orderpt.end(), arrange);
